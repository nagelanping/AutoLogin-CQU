...
一个轻量级、高性能、跨平台的校园网自动认证工具。本项目包含 **Python 通用版** 以及基于系统底层 API 的 **C++ Windows/Linux 高性能版**。

## **项目背景与设计理念**

本项目起源于对网络上开源 Python 登录脚本的需求：ipv6 支持、绕过代理、性能优化、完善处理逻辑、简单易用和跨平台。目标是创建一个**不依赖大型运行时环境**、**配置分离**、且**资源占用极低**的常驻后台工具。

**核心特性：**
- **配置分离**：账号信息和常规设置与核心代码严格分离，配置文件 `config.ini` 独立于可运行文件
- **跨平台支持**：提供 Windows (.exe) 和 Linux 二进制文件
